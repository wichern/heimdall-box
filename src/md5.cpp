...
void MD5::update(const uint8_t* in, uint32_t size)
{
    ...
    count[0] += size << 3;
    if (count[0] < (size << 3)) { // detect overflow
        count[1]++;
    }
    count[1] += size >> 29;
    ...
    uint32_t i = 0;
    if (size >= partLen) {
        std::memcpy(&buffer[index], in, partLen);
    ...
    std::memcpy(&buffer[index], &in[i], size - i);
}

void MD5::update(const std::string& str)
{
    update(reinterpret_cast<const uint8_t*>(str.c_str()), static_cast<uint32_t>(str.size()));
}

void MD5::update(const std::u16string& str)
{
    update(reinterpret_cast<const uint8_t*>(str.c_str()), static_cast<uint32_t>(str.size() * sizeof(char16_t)));
}
